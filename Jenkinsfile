pipeline {
  ...
  environment {
    ...
    SLACK_TEAM   = 'devopsipi'                  // ton workspace (sans .slack.com)
    ...
    stage('Build image (compose)') {
      steps {
        sh '''
          ...
          # Tag versionné robuste ( <commit>-<buildNumber> )
      ...
      script {
        try {
          slackSend(
            ...
            color:      'good',
            botUser:    true,
            message:    "Build & Push OK : ${env.JOB_NAME} #${env.BUILD_NUMBER} -> ${env.REGISTRY_URL}/${env.IMAGE_REPO}:latest",
      ...
      script {
        try {
          slackSend(
            ...
            color:      'danger',
            botUser:    true,
            message:    "Echec pipeline : ${env.JOB_NAME} #${env.BUILD_NUMBER}",
            tokenCredentialId: env.SLACK_CRED
          )
        } catch (Exception e) {
